...
banca <- function(id, xml_data, 
                       ...
                       nome_instituicao, nome_curso, nome_comissao, ano_ini, ano_fim){
  ...
  if(n > 0){
    for(i in (1:n)){
      ...
      if(ano >= ano_ini & ano <= ano_fim){
        ...
        # quando for processo seletivo do PROGRAMA
#        if(nivel == "outra" &
#           (titulo == "Processo seletivo discente do Programa de P?s-Gradua??o em Oceanografia") &
#           instituicao == "Universidade Federal de Santa Catarina"){
#          curso <- "Programa de P?s-Gradua??o em Oceanografia"
#          pontos <- pontos_banca(3)

        # quando for processo seletivo do PROGRAMA
          if(nivel == "outra" &
             (titulo == nome_comissao) &
             instituicao == nome_instituicao[1]) {
            curso <- nome_curso[1]
            pontos <- pontos_banca(3)
